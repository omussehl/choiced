...
const DisneyMovie = () => {
  ...
  const hulu = async () => {
    await fetch("http://localhost:8080/disney/movie")
      .then((res) => {
        return res.json();
      })
      .then((data) => {
  ...
  return (
    <div>
      <h3 className="text-umber">Now search for your random Disney+ Movie</h3>
      <button
        onClick={handleClick}
        type="submit"
        className="bg-disney p-2 rounded-md text-white mb-2"
      >
        Search
      </button>
      <div role="textbox">
